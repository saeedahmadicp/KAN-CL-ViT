...
class KAN(nn.Module):
    ...
    def train2(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
    ...
    def train_single_KAN(self,  timer, optimizer, inputs, labels, args_dict, steps=10, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=False, grid_update_num=10, stop_grid_update_step=50,
        ...
        loss_fn = loss_fn_eval = args_dict['loss_function']
        input_features = args_dict['feature_extractor'](inputs)
        ...
        for _ in pbar:
            ...
            if _ % grid_update_freq == 0 and _ < stop_grid_update_step and update_grid:
                self.update_grid_from_samples(input_features)
        ...
        metric = self.calc_metrics(pred, labels)

        return train_loss.item(), metric, timer.get_elapsed_time()